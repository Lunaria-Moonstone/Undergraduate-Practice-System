...
export async function PUT(request: NextRequest) {
  ...
  try {
    let check: { ok: boolean, results: Array<any> } = await selectSafty({ field: ['*'], table: 'user', where: { id: user.role_id, password: password } }) as { ok: boolean, results: Array<any> } ;
    if (!check.ok || (check.ok && check.results.length === 0)) {
      throw new Error();
    }
    results = await updateSafty({ field: { password: new_password }, table: 'user', where: { id: user.role_id, password: password } });
  } catch (error: unknown) {
    results = error;
  } finally {
