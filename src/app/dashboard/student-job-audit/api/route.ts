...
export async function POST(request: NextRequest) {
  ...
  let id = nanoid();
  let { student_id, company_id, job_id, progress, feedback, resume, created  } = JSON.parse((await request_body.text()));
  return await router.POST({ id, student_id, company_id, job_id, progress, feedback, resume, created  });
}
