...
export async function GET(request: NextRequest) {
  ...
  let user_info: { ok: boolean, results: Array<{ id: string, grade: string }> } = (await selectSafty({ field: [ 'id', 'grade' ], table: role_map[user.role], where: {id: user.role_id}, limit: 1 })) as { ok: boolean, results: Array<{ id: string, grade: string }> };
...
export async function PUT(request: NextRequest ) {
  ...
  if (data['is_practice'])
    data['is_practice'] = Number(data['is_practice']);
  console.log(data);
